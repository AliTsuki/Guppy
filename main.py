...
pat = re.compile(r'([A-Z][^.!?]*[.!?])',
                 re.M)  # Regex pattern for grabbing everthing before a sentence ending punctuation
...
sentenceListForWords = pat.findall(
    rawText)  # Run the regex pattern again this time with the punctuation broken up by spaces
...
for index, words in enumerate(sentenceList):  # Enumerate through sentence list
    ...
    words = sentenceList[sentenceIndex].split(
        ' ')  # Split up the words in each sentence so we have a nested lists that contain each word in each sentence
...
for item in wordList:  # loop through the word list
    lowercaseWordList.append(
        item.lower())  # append the lowercase version of the item in word list to the lowercase word list
...
unorderedWordDoubleListDict = (Counter(zip(lowercaseWordList,
                                           nxt)).items())  # Create a dict using Counter that zips a tuple of
# word list and next item in word list with the number of times that tuple exists in the text
print(unorderedWordDoubleListDict)

# sentenceList = List of all sentences firstWordList = List of words that start sentence list sentenceListForWords =
# List of all sentences mutated for ease of extracting words wordsInSentenceList = List of lists containing all of
# the words in each sentence wordList = List of all words lowercaseWordList = List of all words in all lowercase
# unorderedWordDoubleListDict = Dict of all unique word pairs in a tuple plus the count of times that unique pair
# appears in the text
